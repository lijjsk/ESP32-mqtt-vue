<template>
  <div class="login-wrap">
    <el-form label-position="left" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="0px" class="box">
      <h3 class="register-title">注册</h3>
        ...
        <el-form-item prop="password">
          <el-input type="text" v-model="ruleForm.pass" auto-complete="off" placeholder="密码"></el-input>
        </el-form-item>

        <el-form-item prop="checkPass">
          <el-input type="text" v-model="ruleForm.checkPass" auto-complete="off" placeholder="确认密码"></el-input>
        </el-form-item>
        <el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        name:[
          { min: 3, max: 10, message: '用户名必须是3到10位', trigger: 'blur' },
        ],
        pass: [
          { min: 6, max:20, message: '密码必须是6到20位', trigger: 'blur' },
...
.box {
  background: linear-gradient(to bottom, #e1e8f0, #f5f5f5); /* 调整渐变背景颜色 */
  border: 2px solid #dae1e7; /* 调整边框颜色 */
  width: 400px;
  margin: 10% auto;
  padding: 25px;
  border-radius: 20px;
  box-shadow: 0 0 10px #d0d8e3, 0 0 40px rgba(0, 0, 0, 0.05); /* 调整阴影颜色 */
  position: relative;
}
...
.box:before {
  ...
  left: -2px;
  z-index: -1;
  border-radius: 20px; /* 将伪元素的边框圆角设置为20px */
  box-shadow: 0 0 20px #a1c4fd;
}
